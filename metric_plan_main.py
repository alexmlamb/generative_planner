...
class Planner(nn.Module):
    ...
    def score_traj(self, slst, alst, sg):
        ...
        train_score += self.score.forward_score(slast, sg)[1]
        #train_score += self.score.forward_enc(slast, alst, sg, 2)[1]*0.50

        eval_score += self.score.forward_score(slast, sg)[1]
        ...
        if False:
            ...
            eval_score += self.score.forward_enc(slast, alst, sg, 2)[1] #michael henaff, yann lecun.  Uncertainty-based penalization of model-based RL. curiosity driven exploration
    ...
    def simulate(self, s1, sg, sg_targ, ksim):
        ...
        for j in range(0,ksim):
            ...
            a = self.pvn.mode(torch.cat([s, sg_use, kemb], dim=1))
            s = self.score.forward_pred(s, a)*1.0
    ...
    def multistep(self,s0,a0,sg,sg_pre,ktr):
        ...
        OT_a0 = torch.clamp(self.pvn.mode(torch.cat([s0, sg, kemb], dim=1)),-1*amax,amax)
        OT_s1 = self.score.forward_pred(s0, OT_a0)
        #OT_s1 = dyn(s0, OT_a0)

        RT_a0 = torch.clamp(self.pvn.mode(torch.cat([s0, sg_targ, kemb], dim=1))+torch.randn_like(OT_a0)*0.1, -0.2, 0.2)
        RT_s1 = self.score.forward_pred(s0, RT_a0)
        #RT_s1 = dyn(s0, RT_a0)
        ...
        T_slst, T_alst = self.simulate(T_s1, T_sg, T_sg_targ, ktr-1)
        ...
        a0_pred, kl_loss = self.pvn.train_act(torch.cat([s0.detach(),sg.detach(), kemb], dim=1), a0_targ) #Better to use sg (goal) or what we reached (sk_targ).  
        ...
        slst = torch.cat([s0.unsqueeze(0), OT_slst], dim=0)

        slst = self.score.net_probe(slst)

        return l, a0_pred, slst, OT_score
...
if __name__ == "__main__":
    ...
    for j in range(0,2*300000):
        ...
        if ktr == 1:
            ...
            s0_n = score.forward_pred(score.enc_state(s0), arand)
            #s0_n = dyn(s0, arand)

            arand = torch.clamp(torch.randn_like(s0)*0.1, -0.2,0.2)
            sk_n = score.forward_pred(score.enc_state(sk), arand)
            #sk_n = dyn(sk, arand)

            #arand = torch.clamp(torch.randn_like(s0)*0.1, -0.2,0.2)
            #sk_pre_n = score.forward_pred(score.enc_state(sk_pre), arand)
            #sk_pre_n = dyn(sk_pre, arand)

            s0_enc = score.enc_state(s0)
            sk_enc = score.enc_state(sk)
            sk_pre_enc = score.enc_state(sk_pre)

            a0 = a[:,0]
            l,a0pred,splan, plan_score = plan.multistep(s0_enc,a0,sk_enc,sk_pre_enc,ktr)
